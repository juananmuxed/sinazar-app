<template>
    <v-container fluid>
        <v-layout row wrap align-center justify-end>
            <v-flex xs6 md2 pa-2 class="text-xs-left">Página {{ menu.paginacion.page }} - {{ Math.ceil(juegos.length/menu.paginacion.rowsPerPage) }}</v-flex>
            <v-flex xs6 md1 pa-2>
                <v-select label="Por página" :color="menu.color + selectcolor()" v-model="paginas" :items="[4, 8, 16]"></v-select>
            </v-flex>
            <v-flex xs3 md2 pa-2 class="text-xs-left">Total juegos: {{ juegos.length }}</v-flex>
            <v-flex xs9 md7 pa-2 class="text-xs-right">
        ...
        <v-fade-transition>
            <v-data-iterator :search="busqueda.nombre" hide-actions :pagination.sync="menu.paginacion" :items="juegosfiltrados" content-tag="v-layout" row wrap :rows-per-page-items="[4, 8, 16,{'text':'∞','value':-1}]" no-data-text="Sin juegos que mostrar..." no-results-text="No se encuentran juegos con ese filtro...">
                        ...
                        <v-card>
                            ...
                            <v-card-text>
                                <v-layout pa-1 align-center>
                                    <v-flex xs3><v-avatar size="40" :color="juego.item.colorazar"><span class="title white--text">{{ juego.item.azar }}</span></v-avatar></v-flex>
                                    <v-flex xs9><v-rating dense v-model="juego.item.azar" size="14" length="20" empty-icon="fas fa-dice-one" full-icon="fas fa-dice-six" :readonly="true" :background-color="menu.color + ' lighten-2'" :color="menu.color + ' darken-3'"></v-rating></v-flex>
                                </v-layout>
                                <v-layout pa-1 align-center>
                                    <v-flex xs3><v-avatar size="40" :color="juego.item.colorpierde"><span class="title white--text">{{ juego.item.pierdeamigos }}</span></v-avatar></v-flex>
                                    <v-flex xs9><v-rating dense v-model="juego.item.pierdeamigos" size="14" length="20" empty-icon="far fa-user" full-icon="fas fa-user" :readonly="true" :background-color="menu.color + ' lighten-2'" :color="menu.color + ' darken-3'"></v-rating></v-flex>
                                </v-layout>
                            </v-card-text>
                            <v-card-actions>
